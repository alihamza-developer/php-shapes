...
function generate($spacer = null, $gap = 0)
{
    ...
    // Now first we will call get_path function and get the d path 
    // Then we parce it with preg_replace_all and try to get the L gap_x,gap_y  of the cornors  
    // we just find one cornor and just need to increase this gap in inset_x and inset_y 
    $path = get_path();
    ...
    switch ((int)$count) {
        case 4:
            // Four corners
            $out .= $make(($r + 100) - $width / 1, $r + 100);      // top-left
            // $out .= $make($width - $r, $r);     // top-right
            // $out .= $make($r, $height - $r);   // bottom-left
            // $out .= $make($width -  $r, $height - $r);  // bottom-right
            break;
